...
PATH_BLOCK = Block("minecraft:smooth_quartz")    
...
def pave_village_area(editor, heightmap, rect, placements, padding: int = 2):
    ...
    and leaves grass elsewhere so the village reads as buildings, paths, green"""
    ...
    #Stone-brick plaza around the chapel 
    if chapel_info:
...
def place_harbor(editor, heightmap, rect, build_area, sea_level: int = 63):
    ...
    #Big passenger ferry parked at the end of the dock
...
def place_passenger_ferry(editor, wx, wz, sea_level=63):
    """Build a Greek-island passenger ferry"""
...
def draw_path(editor: Editor, heightmap, rect, start, end):
    ...
    while safety > 0:
        ...
        if 0 <= lx < rect.size.x and 0 <= lz < rect.size.y:
            ...
            if ground_y < 64:        #never draw paths over water
                continue
...
def place_well(editor: Editor, center: ivec3):
    """cobblestone ring with water in the middle and four corner pillars"""
    #Ring
    for dx in range(-1, 2):
        for dz in range(-1, 2):
            if (dx, dz) != (0, 0):
...
def write_text_on_ground(editor: Editor, text: str, start_x: int, start_z: int,
    ...
    """Write text flat on the terrain"""
    SEA_LEVEL = 63
