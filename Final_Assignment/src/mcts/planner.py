...
def get_candidate_positions(heightmap, rect, build_area, step: int = 5, footprint: int = 9):
    """Find buildable positions on the island terrain on a coarse grid."""
    candidates = []
    ...
    for x in range(sx, ex, step):
        for z in range(sz, ez, step):
            ...
            if ground_y < 64:   # below sea level — skip
                continue
...
def is_too_close(x: int, z: int, placements: List[Placement], min_dist: int = 11) -> bool:
